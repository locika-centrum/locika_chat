  ...
  List<TableRow> _tableContent() {
    ...
    for (int i = 0; i < AppSettings().data.gameSizes.length; i++) {
      result.add(TableRow(children: [
        ...
        Center(
          child: Text(
              '${AppSettings().data.getReversiScore(i).noOfWins} : ${AppSettings().data.getReversiScore(i).noOfLosses}'),
        ),
        Center(child: Text('${AppSettings().data.getReversiScore(i).noOfGames}')),
      ]));
      totalWins += AppSettings().data.getReversiScore(i).noOfWins;
      totalLoses += AppSettings().data.getReversiScore(i).noOfLosses;
      totalGames += AppSettings().data.getReversiScore(i).noOfGames;
    }
